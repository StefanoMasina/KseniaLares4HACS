...
from .const import DATA_COORDINATOR, DATA_ZONES, DOMAIN, ZONE_STATUS_ALARM
...
async def async_setup_entry(
    ...
    async_add_entities(
        LaresBinarySensor(coordinator, idx, data["description"])
...
class LaresBinarySensor(CoordinatorEntity, BinarySensorEntity):
    ...
    def __init__(
        self, coordinator: CoordinatorEntity, idx: int, description: str
    ) -> None:
    ...
    def is_on(self):
        ...
        self._ison = (
            self._coordinator.data[DATA_ZONES][self._idx]["allarme"]
            == ZONE_STATUS_ALARM
        )
        return self._ison
