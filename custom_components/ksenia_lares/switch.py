...
from .const import DATA_COORDINATOR, DATA_ZONES, DOMAIN, ZONE_BYPASS_OFF
...
async def async_setup_entry(
    ...
    async_add_entities(
        ZoneBypassSwitch(coordinator, idx, data["description"], data["status"], "Z")
...
class ZoneBypassSwitch(CoordinatorEntity, SwitchEntity):
    ...
    def is_on(self) -> bool:
        ...
        self.___is_on = (
            self.coordinator.data[DATA_ZONES][self._idx.split("Z")[1]]["status"]
            == ZONE_BYPASS_OFF
        )
